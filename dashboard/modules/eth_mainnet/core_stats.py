...
def fetch_data(filters):
    ...
    df_account_delegation = pd.read_sql_query(
        f"""
        SELECT * FROM {st.secrets.database.DB_ENV}_eth_mainnet.fct_core_account_delegation_eth_mainnet
    ...
    df_apr = pd.read_sql_query(
        ...
        FROM {st.secrets.database.DB_ENV}_eth_mainnet.fct_core_apr_eth_mainnet apr
        LEFT JOIN {st.secrets.database.DB_ENV}_seeds.eth_mainnet_tokens tk on lower(apr.collateral_type) = lower(tk.token_address)
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
        and pool_id = 1
        ORDER BY ts
    """,
    ...
    df_apr_token = pd.read_sql_query(
        ...
        SELECT 
            ...
            rewards_usd,
            apr_{resolution}_rewards as apr_rewards
        FROM {st.secrets.database.DB_ENV}_eth_mainnet.fct_core_apr_rewards_eth_mainnet apr
        LEFT JOIN {st.secrets.database.DB_ENV}_seeds.eth_mainnet_tokens tk on lower(apr.collateral_type) = lower(tk.token_address)
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
        and pool_id = 1

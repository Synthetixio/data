...
def fetch_data(settings):
    ...
    df_wrapper = pd.read_sql_query(
        ...
        SELECT
            ...
            synth_market_id,
            amount_wrapped
        FROM prod_eth_mainnet.fct_spot_wrapper_eth_mainnet
    ...
    df_atomics = pd.read_sql_query(
        ...
        SELECT
            ...
            amount,
            price
        FROM prod_eth_mainnet.fct_spot_atomics_eth_mainnet
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
    """,
        db,
    )
    ...
    df_synth_supply = pd.read_sql_query(
        ...
        SELECT
            ...
            synth_market_id,
            supply
        FROM prod_eth_mainnet.fct_synth_supply_eth_mainnet
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
    """,
        db,
    )

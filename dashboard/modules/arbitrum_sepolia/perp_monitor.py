...
def fetch_data(filters):
    ...
    df_skew = pd.read_sql_query(
        ...
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_market_history_arbitrum_sepolia
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
        ORDER BY ts
    """,
        db,
    ...
    df_market = pd.read_sql_query(
        ...
        SELECT
            ...
            exchange_fees,
            liquidations
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_market_stats_{resolution}_arbitrum_sepolia
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
        """,
        db,
    )
    df_stats = pd.read_sql_query(
        ...
        SELECT
            ...
            liquidated_accounts,
            liquidation_rewards
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_stats_{resolution}_arbitrum_sepolia
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
        """,
        db,
    )

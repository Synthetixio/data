...
def fetch_data(filters):
    ...
    df_order = pd.read_sql_query(
        f"""
        SELECT distinct account_id, sender FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_orders_arbitrum_sepolia
    """,
        db,
    )
    df_order_expired = pd.read_sql_query(
        ...
        SELECT
            ...
            acceptable_price,
            commitment_time
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_previous_order_expired_arbitrum_sepolia
    ...
    df_trade = pd.read_sql_query(
        ...
        SELECT
            ...
            accrued_funding,
            tracking_code
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_trades_arbitrum_sepolia
    ...
    df_transfer = pd.read_sql_query(
        ...
        SELECT
            ...
            synth_market_id,
            amount_delta
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_collateral_modified_arbitrum_sepolia
        WHERE account_id = {account_id if account_id else 'NULL'}
        and date(block_timestamp) >= '{start_date}' and date(block_timestamp) <= '{end_date}'
    """,
        db,
    ...
    df_interest = pd.read_sql_query(
        ...
        SELECT
            ...
            cast(account_id as text) as account_id,
            interest
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_interest_charged_arbitrum_sepolia
        WHERE account_id = {account_id if account_id else 'NULL'}
        and date(block_timestamp) >= '{start_date}' and date(block_timestamp) <= '{end_date}'
    """,
        db,
    ...
    df_account_liq = pd.read_sql_query(
        ...
        SELECT
            ...
            account_id,
            total_reward
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_liq_account_arbitrum_sepolia
        WHERE account_id = '{account_id}'
        and ts >= '{start_date}' and ts <= '{end_date}'
    """,
        db,
    ...
    df_hourly = pd.read_sql_query(
        ...
        SELECT
            ...
            cumulative_volume,
            cumulative_fees
        FROM {st.secrets.database.DB_ENV}_arbitrum_sepolia.fct_perp_account_stats_hourly_arbitrum_sepolia
        WHERE account_id = '{account_id}'
        and ts >= '{start_date}' and ts <= '{end_date}'
        order by ts
    """,

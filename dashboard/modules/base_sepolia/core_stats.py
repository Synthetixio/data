...
def fetch_data():
    ...
    df_collateral = pd.read_sql_query(
        f"""
        SELECT * FROM base_sepolia.fct_core_pool_collateral
    """,
        db,
    )
    ...
    df_delegation = pd.read_sql_query(
        f"""
        SELECT * FROM base_sepolia.fct_core_pool_delegation
    """,
        db,
    )
    ...
    df_market_updated = pd.read_sql_query(
        f"""
        SELECT * FROM base_sepolia.fct_core_market_updated order by ts
    """,
        db,
    )
    ...
    df_pnl = pd.read_sql_query(
        f"""
        SELECT * FROM base_sepolia.fct_perp_pnl order by ts
    """,
        db,
    )

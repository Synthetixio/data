...
def fetch_data(filters):
    ...
    df_collateral = pd.read_sql_query(
        f"""
        SELECT ts, collateral_type, amount_deposited FROM base_sepolia.fct_core_pool_collateral
    ...
    df_delegation = pd.read_sql_query(
        f"""
        SELECT ts, pool_id, collateral_type, amount_delegated FROM base_sepolia.fct_core_pool_delegation
        WHERE ts >= '{start_date}' and ts <= '{end_date}'
    """,
        db,
    )

...
def fetch_data(filters):
    ...
    df_order_expired = pd.read_sql_query(
        ...
        SELECT
            ...
            acceptable_price,
            commitment_time
        FROM prod_base_mainnet.perp_previous_order_expired_base_mainnet
    ...
    df_transfer = pd.read_sql_query(
        ...
        SELECT
            ...
            synth_market_id,
            amount_delta
        FROM prod_base_mainnet.perp_collateral_modified_base_mainnet
        WHERE account_id = {account_id if account_id else 'NULL'}
        and date(block_timestamp) >= '{start_date}' and date(block_timestamp) <= '{end_date}'
    """,
        db,
    ...
    df_interest = pd.read_sql_query(
        ...
        SELECT
            ...
            cast(account_id as text) as account_id,
            interest
        FROM prod_base_mainnet.perp_interest_charged_base_mainnet
        WHERE account_id = {account_id if account_id else 'NULL'}
        and date(block_timestamp) >= '{start_date}' and date(block_timestamp) <= '{end_date}'
    """,
        db,

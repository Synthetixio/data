...
def fetch_data(filters):
    ...
    df_order = pd.read_sql_query(
        f"""
        SELECT distinct account_id, sender FROM base_mainnet.fct_perp_orders
    """,
        db,
    )
    ...
    df_account_liq = pd.read_sql_query(
        ...
        SELECT
            ...
            account_id,
            total_reward
        FROM base_mainnet.fct_perp_liq_account
        WHERE account_id = '{account_id}'
        and ts >= '{start_date}' and ts <= '{end_date}'
    """,
        db,
    ...
    df_hourly = pd.read_sql_query(
        ...
        SELECT
            ...
            cumulative_volume,
            cumulative_fees
        FROM base_mainnet.fct_perp_account_stats_hourly
        WHERE account_id = '{account_id}'
        and ts >= '{start_date}' and ts <= '{end_date}'
        order by ts
    """,

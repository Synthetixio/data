...
def fetch_data():
    ...
    df_order_expired = pd.read_sql_query(
        "SELECT * FROM base_goerli.perp_previous_order_expired", db
    )
    df_trade = pd.read_sql_query("SELECT * FROM base_goerli.fct_perp_trades", db)
    df_market_updated = pd.read_sql_query(
        "SELECT * FROM base_goerli.fct_perp_market_history", db
    )
    df_account_liq = pd.read_sql_query(
        "SELECT * FROM base_goerli.fct_perp_liq_account", db
    )

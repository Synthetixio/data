    ...
    select
        arrayJoin(
            arrayMap(
                x -> toDateTime(x),
                range(
                    toUInt32(date_trunc('day', date(min(activity_date)))),
                    toUInt32(date_trunc('day', date(max(activity_date))))
                )
            )
        ) as activity_date
...
active_accounts_monthly as (
    ...
    from date_range
    left join active_accounts
        on active_accounts.activity_date between 
            date_sub(day, 27, date_range.activity_date) and 
            date_range.activity_date
    group by date_range.activity_date
),
